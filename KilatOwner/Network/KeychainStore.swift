...
final class KeychainStore: TokenStore {
    ...
    init(serviceIdentifier: String = Bundle.main.bundleIdentifier ?? "my.kilat.KilatOwner") {
    ...
    private func save(_ token: String, for key: TokenKey) throws {
        guard let data = token.data(using: .utf8) else {
            throw APIError.encoding("Token was not valid UTF-8.")
        }
        ...
        guard status == errSecSuccess else {
            throw APIError.network("Could not save credentials.")
        }
    }
